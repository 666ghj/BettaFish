...
Handles Azure-specific requirements:
- api_version parameter (required by Azure)
- Azure endpoint format
- Deployment name as model_name
"""
...
class AzureOpenAIAdapter(BaseLLMClient):
    ...
    - azure_endpoint: The Azure resource endpoint
    - api_version: API version string (e.g., "2024-02-01")
    - model_name: The deployment name (not model family)
    """
    ...
    def __init__(
        ...
        Args:
            ...
            base_url: Azure endpoint (e.g., https://resource.openai.azure.com)
            api_version: API version string (default: 2024-02-01)
            timeout: Request timeout in seconds (default: 1800)
        """
        if AzureOpenAI is None:
            raise ImportError(
                "AzureOpenAI not available. Install with: pip install openai>=1.0.0"
            )

        if not api_key:
            raise ValueError("API key is required for Azure OpenAI adapter")
        if not model_name:
            raise ValueError("Deployment name is required for Azure OpenAI adapter")
        if not base_url:
        ...
        self.api_version = api_version or os.getenv(
            "AZURE_API_VERSION", self.DEFAULT_API_VERSION
        )
    ...
    def invoke(self, system_prompt: str, user_prompt: str, **kwargs) -> str:
        """
        Non-streaming call to Azure OpenAI.
    ...
    def stream_invoke(
        ...
        """
        Streaming call to Azure OpenAI.
        ...
        try:
            ...
            logger.error(f"Azure streaming request failed: {str(e)}")
            raise
