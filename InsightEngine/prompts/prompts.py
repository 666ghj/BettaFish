...
2. **深度理解段落需求**：根据段落主题，思考需要了解哪些具体的公众观点和情感
3. **精准选择查询工具**：选择最能获取真实民意数据的工具
4. **设计接地气的搜索词**：**这是最关键的环节！**
   - **🔴 强制要求：搜索词必须直接使用query字段的原词**：不要用更具体的子话题、品牌名、型号名、事件名替换原词。保持查询的通用性，不要缩小范围
...
- ✅ 正确：直接使用具体名称、简称或昵称
- ❌ 错误："XX事件 用户反应"
- ✅ 正确："XX出事" 或 "XX怎么了" 或 相关热点词汇

**不同平台语言特色参考**：
- **微博**：热搜词汇、话题标签
- **知乎**：问答式表达，如 "如何看待XX"
- **B站**：弹幕文化，常用"yyds"、"路过"等网络用语
- **贴吧**：直接称呼，社区化表达
- **抖音/快手**：短视频描述，如 "XX日常"、"XX vlog"
- **小红书**：分享式，如 "XX真的很XX"、"XX攻略"
