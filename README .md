...
- 确保本地已安装 **Docker Engine 24+** 与 **Docker Compose v2**。
- 克隆或下载项目代码后，在根目录执行后续命令。
...
2. 打开新生成的 `.env` 文件，至少完成以下修改：

   | 配置项 | 推荐值 | 说明 |
   | :--- | :--- | :--- |
   | `DB_HOST` | `db` | 与 `docker-compose.yml` 中的数据库服务名保持一致 |
   | `DB_PORT` | `5432` | 容器内数据库端口，宿主机默认映射到 `5444` |
   | `DB_USER`/`DB_PASSWORD`/`DB_NAME` | `bettafish` | 与 `docker-compose.yml` 中的 `POSTGRES_*` 变量匹配 |
   | `DB_DIALECT` | `postgresql` | 与容器中运行的数据库类型保持一致 |

   其他 LLM 与第三方服务参数请根据自身账号配置，保持 OpenAI 接口兼容即可。
...
> 如需加速镜像拉取，可将 `docker-compose.yml` 中已注释的镜像地址替换为本地可访问的镜像源。

初次启动会自动创建数据库并初始化目录（如 `logs/`、`final_reports/`）。首次运行完成后可通过以下命令确认服务状态：
...
当日志出现 `Running on http://0.0.0.0:5000` 时，即可在浏览器访问 `http://localhost:5000` 体验全功能系统。

> **重要说明**：容器编排仅包含 Web 应用与数据库服务，**数据库爬取（MindSpider）需按照源码部署指南 5.3 节单独启动**，以免遗漏实时数据更新。

### 4. 常用运维操作

- **停止服务**：`docker compose down`
- **重启服务**：`docker compose restart`
- **更新镜像后重启**：`docker compose pull && docker compose up -d`

所有业务数据（报告、日志、数据库数据等）均通过卷挂载到本地目录，升级镜像不会影响历史数据。
...
- `.env` 已根据上一节要求填写完成，并与数据库/LLM 服务连通；
- 本地或远程数据库已经启动（Docker 部署时由 `docker compose` 自动拉起）；
- 项目根目录下的虚拟环境已安装所有依赖。

#### 5.1 使用 Flask 主应用启动（推荐）

```bash
# 进入项目根目录并激活虚拟环境
conda activate your_conda_name  # 或 source .venv/bin/activate

# 运行主程序，默认读取 .env 中的 HOST 与 PORT
python app.py
```

如需在服务器上对外提供服务，可执行 `python app.py --host 0.0.0.0 --port 5000`，或在 `.env` 中修改 `HOST`、`PORT`。
...
当终端输出 `Running on http://0.0.0.0:5000` 时，即可访问 `http://localhost:5000`（或自定义地址）体验完整系统。
